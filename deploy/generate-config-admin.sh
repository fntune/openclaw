#!/bin/bash
# Generate final openclaw.json from admin template + env secrets.
# Expects /opt/openclaw/.env to exist (run fetch-secrets-admin.sh first).
set -euo pipefail
...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "ERROR: $ENV_FILE not found — run fetch-secrets-admin.sh first" >&2
  exit 1
fi
